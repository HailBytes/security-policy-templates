...
#   --ticketing-system "Jira"             [approved change management tool or ticketing system]
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --ticketing-system) TICKETING_SYSTEM="$2"; shift 2 ;;
...
build_sed_args() {
  ...
  [[ -n "$TICKETING_SYSTEM" ]] && args+=(-e "s/\[approved change management tool or ticketing system\]/$(escape_replace "$TICKETING_SYSTEM")/g")
...
[[ -n "$TICKETING_SYSTEM" ]] && echo "  [approved change management tool or ticketing system] → $TICKETING_SYSTEM"
echo ""
...
for src_file in "${TARGET_FILES[@]}"; do
  ...
  if ! grep -qE '\[(Company Name|Date|IT Contact|Security Officer|Review Date|Department|Industry|Title/Role|Contact Info|Day of week|Time range|Primary business application|Secondary applications|approved change management tool or ticketing system)\]' "$src_file" 2>/dev/null; then
