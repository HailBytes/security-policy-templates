...
#   --cm-tool      "Jira Service Mgmt"   [Change Management Tool]
...
#     --cm-tool "Jira Service Management" \
...
build_sed_args() {
  ...
  [[ -n "$CM_TOOL" ]]         && args+=(-e "s/\[Change Management Tool\]/$(escape_replace "$CM_TOOL")/g")
...
[[ -n "$CM_TOOL" ]]        && echo "  [Change Management Tool]       → $CM_TOOL"
echo ""
...
for src_file in "${TARGET_FILES[@]}"; do
  ...
  if ! grep -qE '\[(Company Name|Date|IT Contact|Security Officer|Review Date|Department|Industry|Title/Role|Contact Info|Day of week|Time range|Primary business application|Secondary applications|Change Management Tool)\]' "$src_file" 2>/dev/null; then
...
if ! $DRY_RUN; then
  ...
  remaining=$(grep -rn --include="*.md" \
    -E '\[(Company Name|Date|IT Contact|Security Officer|Review Date|Department|Industry|Title/Role|Contact Info|Day of week|Time range|Primary business application|Secondary applications|Change Management Tool)\]' \
